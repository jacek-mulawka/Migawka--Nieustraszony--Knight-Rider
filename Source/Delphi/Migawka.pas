...
type
  ...
  private
    ...
    elementy_iloœæ,
    opóŸnienie_zmiany, // Po ilu cyklach œwiate³ko zawraca (skrajne siê œwieci). // 6.
    pozycja_œwiec¹ca,
    prêdkoœæ // 50.
      : integer;
...
constructor TMigawka.Create( rodzic_f : TWinControl; pozycja__lewo_f, pozycja__góra_f, elementy_iloœæ_f, prêdkoœæ_f, opóŸnienie_zmiany_f : integer );
var
  i : integer;
begin
  ...
  if Self.kierunek = 'P' then
    begin
      ...
      if Self.pozycja_œwiec¹ca <= Self.elementy_iloœæ then
        Self.pasek_LED[ Self.pozycja_œwiec¹ca ].Status := true;

      if Self.pozycja_œwiec¹ca <= Self.elementy_iloœæ then
        Self.pasek_LED[ Self.pozycja_œwiec¹ca - 1 ].Status := false; // Skrajna nie gaœnie podczas odbijania (wygasza).

      // Wygaszanie.
      if    ( Self.pozycja_œwiec¹ca >= 1 )
        and ( Self.pozycja_œwiec¹ca <= Self.elementy_iloœæ + 1 ) then
        Self.pasek_LED[ Self.pozycja_œwiec¹ca - 1 ].ColorOff := $000000E0;

      if    ( Self.pozycja_œwiec¹ca >= 2 )
        and ( Self.pozycja_œwiec¹ca <= Self.elementy_iloœæ + 2 ) then
        Self.pasek_LED[ Self.pozycja_œwiec¹ca - 2 ].ColorOff := $000000C0;

      if    ( Self.pozycja_œwiec¹ca >= 3 )
        and ( Self.pozycja_œwiec¹ca <= Self.elementy_iloœæ + 3 ) then
        Self.pasek_LED[ Self.pozycja_œwiec¹ca - 3 ].ColorOff := $000000A0;

     if    ( Self.pozycja_œwiec¹ca >= 4 )
       and ( Self.pozycja_œwiec¹ca <= Self.elementy_iloœæ + 4 ) then
        Self.pasek_LED[ Self.pozycja_œwiec¹ca - 4 ].ColorOff := clMaroon;
     //---// Wygaszanie.
...
procedure TMigawka.OpóŸnienie_Zmiany_Ustaw( opóŸnienie_zmiany_f : integer );
begin
...
//Predkoœæ_Ustaw().
procedure TMigawka.Predkoœæ_Ustaw( prêdkoœæ_f : integer );
begin
